...
class Tracer {
  ...
  Tracer({required this.name, required this.provider});

  /// Start a new span
  Span startSpan(String name, {SpanKind kind = SpanKind.internal, Map<String, dynamic>? attributes, List<SpanLink>? links}) {
    final parentSpan = provider.activeSpan;

    final span = Span(traceId: parentSpan?.traceId, parentSpanId: parentSpan?.spanId, name: name, kind: kind);
  ...
  Future<T> withSpan<T>(String name, Future<T> Function(Span span) fn, {SpanKind kind = SpanKind.internal, Map<String, dynamic>? attributes}) async {
  ...
  T withSpanSync<T>(String name, T Function(Span span) fn, {SpanKind kind = SpanKind.internal, Map<String, dynamic>? attributes}) {
    final span = startSpan(name, kind: kind, attributes: attributes);
