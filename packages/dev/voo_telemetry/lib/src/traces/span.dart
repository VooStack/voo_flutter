...
class Span {
  ...
  Span({String? traceId, String? spanId, this.parentSpanId, required this.name, this.kind = SpanKind.internal, DateTime? startTime, SpanContext? context})
    : traceId = traceId ?? _generateTraceId(),
      spanId = spanId ?? _generateSpanId(),
      startTime = startTime ?? DateTime.now(),
      context = context ?? SpanContext(traceId: traceId ?? _generateTraceId(), spanId: spanId ?? _generateSpanId());
  ...
  Map<String, dynamic> toOtlp() => {
    ...
    'attributes': attributes.entries.map((e) => {'key': e.key, 'value': _convertAttributeValue(e.value)}).toList(),
...
class SpanEvent {
  ...
  SpanEvent({required this.name, required this.timestamp, required this.attributes});
...
class SpanStatus {
  ...
  Map<String, dynamic> toOtlp() => {'code': code.value, if (description != null) 'message': description};
}
