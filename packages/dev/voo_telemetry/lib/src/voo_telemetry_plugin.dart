...
class VooTelemetryPlugin extends VooPlugin {
  ...
  static Future<void> initialize({
    ...
    if (!Voo.isInitialized) {
      throw const VooException('Voo.initializeApp() must be called before initializing VooTelemetry', code: 'core-not-initialized');
    }
  ...
  FutureOr<void> onAppInitialized(VooApp app) async {
    if (_endpoint == null) {
      throw const VooException('VooTelemetry endpoint not configured', code: 'endpoint-not-configured');
    }
  ...
  VooTelemetry? getTelemetryForApp([String appName = '[DEFAULT]']) => _appTelemetry[appName];

  /// Get the default telemetry instance.
  VooTelemetry? get defaultTelemetry => _appTelemetry['[DEFAULT]'];

  @override
  dynamic getInstanceForApp(VooApp app) => _appTelemetry[app.name];
  ...
  void recordException(dynamic exception, StackTrace? stackTrace, {Map<String, dynamic>? attributes}) {
    if (VooTelemetry.isInitialized) {
      VooTelemetry.instance.recordException(exception, stackTrace, attributes: attributes);
    }
  }
  ...
  @override
  Map<String, dynamic> getInfo() => {...super.getInfo(), 'initialized': _initialized, 'endpoint': _endpoint, 'apps': _appTelemetry.keys.toList()};
}
