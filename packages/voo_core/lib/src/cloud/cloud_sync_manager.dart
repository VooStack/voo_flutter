  ...
  Future<void> sync() async {
    ...
    try {
      ...
      for (final item in batch) {
        ...
        if (type == 'log') {
          logItems.add(item);
        } else if (type == 'metric' || type == 'performance' || type == 'performance_trace' || type == 'network_metric') {
      ...
      if (logItems.isNotEmpty) {
        ...
        final response = await _httpClient!.post(Uri.parse(url), headers: headers, body: jsonEncode(requestBody));
      ...
      if (metricItems.isNotEmpty) {
        ...
        final requestBody = {
          ...
          'metrics': metricItems.map((e) {
            final data = e.data;
            // Handle performance traces and network metrics
            if (e.type == 'performance_trace' || e.type == 'network_metric') {
              final tags = <String, String>{};

              // Add attributes as tags (convert to strings)
              ...
              if (data['method'] != null) tags['method'] = data['method'].toString();
              if (data['statusCode'] != null) tags['statusCode'] = data['statusCode'].toString();
              if (data['url'] != null) tags['url'] = data['url'].toString();

              return {
                'name': data['name'] ?? data['url'] ?? 'unknown',
                'value': (data['duration_ms'] ?? 0).toDouble(),
                'timestamp': e.timestamp.toIso8601String(),
                'type': 'timer',
                'unit': 'ms',
                'tags': tags,
