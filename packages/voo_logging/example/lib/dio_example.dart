...
class _DioExampleScreenState extends State<DioExampleScreen> {
  ...
  void _setupDio() {
    ...
    _dio.interceptors.add(InterceptorsWrapper(onRequest: vooInterceptor.onRequest, onResponse: vooInterceptor.onResponse, onError: vooInterceptor.onError));
  ...
  Future<void> _makeSuccessfulRequest() async {
    ...
    try {
      ...
      // Log success
      await VooLogger.info('Successfully fetched user data');
    } catch (e) {
      setState(() {
        _responseText = 'Error: $e';
      });
      await VooLogger.error('Failed to fetch user data', error: e);
    } finally {
      setState(() {
        _isLoading = false;
      });
  ...
  Future<void> _makeFailedRequest() async {
    ...
    try {
      ...
      await VooLogger.error('Expected 404 error caught', error: e);
    } catch (e) {
      setState(() {
        _responseText = 'Unexpected error: $e';
      });
      await VooLogger.error('Unexpected error', error: e);
    } finally {
      setState(() {
        _isLoading = false;
      });
  ...
  Future<void> _makePostRequest() async {
    ...
    try {
      final response = await _dio.post(
        ...
        data: {'title': 'Test Post from VooLogging', 'body': 'This is a test post to demonstrate VooDioInterceptor', 'userId': 1},
      );
      ...
      await VooLogger.info('Successfully created post', metadata: {'postId': response.data['id']});
    } catch (e) {
      setState(() {
        _responseText = 'Error: $e';
      });
      await VooLogger.error('Failed to create post', error: e);
    } finally {
      setState(() {
        _isLoading = false;
      });
  ...
  Future<void> _makeMultipleRequests() async {
    ...
    try {
      ...
      await VooLogger.info('Successfully completed multiple requests');
    } catch (e) {
      setState(() {
        _responseText = 'Error in one or more requests: $e';
      });
      await VooLogger.error('Failed to complete all requests', error: e);
    } finally {
      setState(() {
        _isLoading = false;
      });
  ...
  Future<void> _makeTimeoutRequest() async {
    ...
    try {
      ...
      final timeoutDio = Dio(BaseOptions(connectTimeout: const Duration(milliseconds: 100), receiveTimeout: const Duration(milliseconds: 100)));

      // Add VooDioInterceptor
      final vooInterceptor = VooDioInterceptor();
      timeoutDio.interceptors.add(
      ...
      if (e.type == DioExceptionType.connectionTimeout || e.type == DioExceptionType.receiveTimeout) {
        ...
        await VooLogger.warning('Request timed out', metadata: {'type': e.type.toString(), 'message': e.message});
      } else {
        setState(() {
          _responseText = 'Different error: ${e.type} - ${e.message}';
        });
        await VooLogger.error('Request failed', error: e);
      }
    } finally {
      setState(() {
        _isLoading = false;
  ...
  Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(title: const Text('Dio Interceptor Example'), backgroundColor: Theme.of(context).colorScheme.inversePrimary),
    body: Padding(
      padding: const EdgeInsets.all(16.0),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: [
          const Text('VooDioInterceptor Demo', style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold)),
          ...
          Container(
            ...
            child: SingleChildScrollView(child: _isLoading ? const Center(child: CircularProgressIndicator()) : Text(_responseText)),
          ),
          const SizedBox(height: 16),
              ...
              children: [
                ...
                ElevatedButton.icon(
                  onPressed: () {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Open Flutter DevTools to see VooLogger logs')));
                  },
...
void setupDioWithVooLogging() {
  ...
  dio.interceptors.add(InterceptorsWrapper(onRequest: vooInterceptor.onRequest, onResponse: vooInterceptor.onResponse, onError: vooInterceptor.onError));
