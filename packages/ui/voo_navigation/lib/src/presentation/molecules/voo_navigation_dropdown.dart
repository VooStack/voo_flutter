...
class _VooNavigationDropdownState extends State<VooNavigationDropdown>
  ...
  Widget build(BuildContext context) {
    ...
    final hasSelectedChild = widget.item.children?.any(
      (child) => child.id == widget.selectedId,
    ) ?? false;
  ...
  Widget _buildHeader(ThemeData theme, bool hasSelectedChild) {
    ...
    final isHighlighted = hasSelectedChild || widget.item.id == widget.selectedId;
    
    return InkWell(
      onTap: widget.item.isEnabled ? _handleTap : null,
      borderRadius: BorderRadius.circular(8),
      child: Padding(
        padding: widget.tilePadding ?? const EdgeInsets.symmetric(
          horizontal: 16,
          vertical: 12,
        ),
        child: Row(
                ...
                style: isHighlighted
                    ? (widget.item.selectedLabelStyle ?? 
                       theme.textTheme.bodyLarge!.copyWith(
                         color: widget.config.selectedItemColor ?? 
                                colorScheme.primary,
                         fontWeight: FontWeight.w600,
                       ))
                    : (widget.item.labelStyle ?? 
                       theme.textTheme.bodyLarge!.copyWith(
                         color: widget.config.unselectedItemColor ?? 
                                colorScheme.onSurfaceVariant,
                       )),
                child: Text(
                  widget.item.label,
  ...
  List<Widget> _buildChildren(ThemeData theme) {
    ...
    for (int i = 0; i < items.length; i++) {
      ...
      children.add(
        _buildChildItem(child, isSelected, theme),
      );
    }
  ...
  Widget _buildChildItem(
    ...
    return InkWell(
      ...
      borderRadius: BorderRadius.circular(8),
      child: Container(
        padding: widget.childrenPadding ?? const EdgeInsets.only(
          left: 48,
          right: 16,
          top: 8,
          bottom: 8,
