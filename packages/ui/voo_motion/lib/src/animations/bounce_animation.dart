...
class VooBounceAnimation extends StatefulWidget {
  ...
  const VooBounceAnimation({super.key, required this.child, this.config = const VooAnimationConfig(), this.bounceHeight = 20, this.numberOfBounces = 3});

  @override
  State<VooBounceAnimation> createState() => _VooBounceAnimationState();
}

class _VooBounceAnimationState extends State<VooBounceAnimation> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _animation = Tween<double>(begin: 0, end: widget.bounceHeight).animate(CurvedAnimation(parent: _controller, curve: curve));
