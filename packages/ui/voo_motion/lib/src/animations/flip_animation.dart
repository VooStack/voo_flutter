...
class VooFlipAnimation extends StatefulWidget {
  ...
  const VooFlipAnimation({super.key, required this.child, this.config = const VooAnimationConfig(), this.direction = FlipDirection.horizontal});

  @override
  State<VooFlipAnimation> createState() => _VooFlipAnimationState();
}

class _VooFlipAnimationState extends State<VooFlipAnimation> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _animation = Tween<double>(begin: 0, end: math.pi).animate(CurvedAnimation(parent: _controller, curve: widget.config.curve));
    ...
    builder: (context, child) {
      ...
      return Transform(
        ...
        transform: Matrix4.identity()
          ...
          ..rotateX(widget.direction == FlipDirection.vertical ? _animation.value : 0)
          ..rotateY(widget.direction == FlipDirection.horizontal ? _animation.value : 0),
        child: isShowingFront
            ? widget.child
            : Transform(
                alignment: Alignment.center,
                transform: Matrix4.identity()
                  ..rotateX(widget.direction == FlipDirection.vertical ? math.pi : 0)
                  ..rotateY(widget.direction == FlipDirection.horizontal ? math.pi : 0),
                child: widget.child,
              ),
      );
    },
