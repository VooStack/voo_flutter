...
enum StaggerAnimationType { fadeIn, slideLeft, slideRight, slideTop, slideBottom, scale }
...
class _VooStaggerListState extends State<VooStaggerList> {
  ...
  Widget build(BuildContext context) {
    ...
    for (int i = 0; i < widget.children.length; i++) {
      ...
      final config = widget.config.copyWith(delay: widget.config.delay + delay);

      Widget animatedChild;

      switch (widget.animationType) {
        case StaggerAnimationType.fadeIn:
          animatedChild = VooFadeAnimation(config: config, child: widget.children[i]);
          break;
        case StaggerAnimationType.slideLeft:
          animatedChild = VooSlideAnimation(config: config, child: widget.children[i]);
          break;
        case StaggerAnimationType.slideRight:
          animatedChild = VooSlideAnimation(config: config, fromOffset: const Offset(1, 0), child: widget.children[i]);
          break;
        case StaggerAnimationType.slideTop:
          animatedChild = VooSlideAnimation(config: config, fromOffset: const Offset(0, -1), child: widget.children[i]);
          break;
        case StaggerAnimationType.slideBottom:
          animatedChild = VooSlideAnimation(config: config, fromOffset: const Offset(0, 1), child: widget.children[i]);
          break;
        case StaggerAnimationType.scale:
          animatedChild = VooScaleAnimation(config: config, child: widget.children[i]);
          break;
      }
