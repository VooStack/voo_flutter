  ...
  group('VooDataGridState Primary Filter Tests', () {
    group('primary filter fields', () {
      ...
      test('should set primaryFilterField', () {
        // Arrange & Act
        const state = VooDataGridState<Map<String, dynamic>>(primaryFilterField: 'status');
      ...
      test('should set primaryFilter', () {
        // Arrange & Act
        const filter = VooDataFilter(operator: VooFilterOperator.equals, value: 'active');
        const state = VooDataGridState<Map<String, dynamic>>(primaryFilter: filter);
      ...
      test('should update primaryFilterField via copyWith', () {
        ...
        // Act
        final updatedState = initialState.copyWith(primaryFilterField: 'category');
      ...
      test('should update primaryFilter via copyWith', () {
        // Arrange
        const initialState = VooDataGridState<Map<String, dynamic>>();
        const filter = VooDataFilter(operator: VooFilterOperator.contains, value: 'test');

        // Act
        final updatedState = initialState.copyWith(primaryFilter: filter);
      ...
      test('should preserve other state when updating primary filter fields', () {
        ...
        const initialState = VooDataGridState<Map<String, dynamic>>(
          ...
          totalRows: 100,
          isLoading: true,
          filters: {'name': VooDataFilter(operator: VooFilterOperator.contains, value: 'John')},
        );

        const primaryFilter = VooDataFilter(operator: VooFilterOperator.equals, value: 'premium');

        // Act
        final updatedState = initialState.copyWith(primaryFilterField: 'tier', primaryFilter: primaryFilter);
      ...
      test('should handle both primary and regular filters independently', () {
        // Arrange
        const regularFilter = VooDataFilter(operator: VooFilterOperator.contains, value: 'test');
        const primaryFilter = VooDataFilter(operator: VooFilterOperator.equals, value: 'active');

        // Act
        const state = VooDataGridState<Map<String, dynamic>>(filters: {'name': regularFilter}, primaryFilterField: 'status', primaryFilter: primaryFilter);
      ...
      test('should preserve primary filter when not specified in copyWith', () {
        // Arrange
        const filter = VooDataFilter(operator: VooFilterOperator.equals, value: 'active');
        const initialState = VooDataGridState<Map<String, dynamic>>(primaryFilterField: 'status', primaryFilter: filter);

        // Act - Not specifying primary filter fields in copyWith
        final updatedState = initialState.copyWith(currentPage: 2);
      ...
      test('should maintain primary filter when regular filters change', () {
        // Arrange
        const primaryFilter = VooDataFilter(operator: VooFilterOperator.equals, value: 'premium');
        const initialState = VooDataGridState<Map<String, dynamic>>(primaryFilterField: 'tier', primaryFilter: primaryFilter);
    ...
    group('primary filter integration', () {
      test('should handle primary filter in combined mode', () {
        ...
        const state = VooDataGridState<Map<String, dynamic>>(
          filters: {
            ...
            'name': VooDataFilter(operator: VooFilterOperator.contains, value: 'test'),
          },
      ...
      test('should handle primary filter in separate mode', () {
        ...
        const state = VooDataGridState<Map<String, dynamic>>(
          filters: {'name': VooDataFilter(operator: VooFilterOperator.contains, value: 'test')},
          primaryFilterField: 'status',
          primaryFilter: primaryFilter,
        );
      ...
      test('should prevent duplication when same field is updated', () {
        // Arrange
        const oldFilter = VooDataFilter(operator: VooFilterOperator.equals, value: 'inactive');
        const newFilter = VooDataFilter(operator: VooFilterOperator.equals, value: 'active');

        const initialState = VooDataGridState<Map<String, dynamic>>(filters: {'status': oldFilter}, primaryFilterField: 'status', primaryFilter: oldFilter);
