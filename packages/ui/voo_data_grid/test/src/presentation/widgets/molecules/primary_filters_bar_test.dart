  ...
  group('PrimaryFiltersBar', () {
    ...
    setUp(() {
      testFilters = [
        ...
        const PrimaryFilter(
          field: 'status',
          label: 'Pending',
          filter: VooDataFilter(operator: VooFilterOperator.equals, value: 'pending'),
        ),
      ];
    });
    ...
    testWidgets('displays all filter buttons', (WidgetTester tester) async {
      ...
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(body: PrimaryFiltersBar(filters: testFilters)),
        ),
      );
    ...
    testWidgets('highlights selected filter', (WidgetTester tester) async {
      // Arrange
      const selectedFilter = VooDataFilter(operator: VooFilterOperator.equals, value: 'active');

      // Act
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(
            body: PrimaryFiltersBar(filters: testFilters, selectedFilter: selectedFilter),
          ),
        ),
      );

      // Assert
      final activeButton = find.byWidgetPredicate((widget) => widget is PrimaryFilterButton && widget.label == 'Active' && widget.isSelected == true);
    ...
    testWidgets('hides All option when showAllOption is false', (WidgetTester tester) async {
      ...
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(body: PrimaryFiltersBar(filters: testFilters, showAllOption: false)),
        ),
      );
    ...
    testWidgets('uses custom All option label', (WidgetTester tester) async {
      ...
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(
            body: PrimaryFiltersBar(filters: testFilters, allOptionLabel: 'Show Everything'),
          ),
        ),
      );
    ...
    testWidgets('handles empty filters list gracefully', (WidgetTester tester) async {
      ...
      await tester.pumpWidget(
        const MaterialApp(
          home: Scaffold(body: PrimaryFiltersBar(filters: [])),
        ),
      );
    ...
    testWidgets('scrolls horizontally when filters overflow', (WidgetTester tester) async {
      ...
      final manyFilters = List.generate(
        ...
        (index) => PrimaryFilter(
          field: 'status',
          label: 'Filter $index',
          filter: VooDataFilter(operator: VooFilterOperator.equals, value: 'value_$index'),
        ),
      );
      ...
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(body: PrimaryFiltersBar(filters: manyFilters)),
        ),
      );
