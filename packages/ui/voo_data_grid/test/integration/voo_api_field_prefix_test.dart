  ...
  group('VooApiStandard Field Prefix PascalCase', () {
    test('should capitalize field names after prefix in VooApiStandard', () {
      ...
      final request = builder.buildRequest(
        ...
        pageSize: 20,
        filters: {
          'siteNumber': const VooDataFilter(value: 100, operator: VooFilterOperator.equals),
          'siteName': const VooDataFilter(value: 'Test', operator: VooFilterOperator.contains),
        },
    ...
    test('should handle number range with PascalCase fields', () {
      ...
      final request = builder.buildRequest(
        page: 0,
        pageSize: 20,
        filters: {'siteNumber': const VooDataFilter(value: 100, valueTo: 200, operator: VooFilterOperator.between)},
        sorts: [],
      );
    ...
    test('should not capitalize for other API standards', () {
      const builder = DataGridRequestBuilder(standard: ApiFilterStandard.jsonApi, fieldPrefix: 'site');

      final params = builder.buildRequest(
        page: 0,
        pageSize: 20,
        filters: {'siteNumber': const VooDataFilter(value: 100, operator: VooFilterOperator.equals)},
        sorts: [],
      );
