...
class _LocalDataGridSource extends VooDataGridSource {
  _LocalDataGridSource({List<dynamic>? data}) : super(mode: VooDataGridMode.local) {
    if (data != null) {
      setLocalData(data);
    }
  }
  ...
  }) async => VooDataGridResponse(rows: [], totalRows: 0, page: page, pageSize: pageSize);
}
...
Widget apiOutputSimpleRest() {
  ...
  const requestBuilder = DataGridRequestBuilder(standard: ApiFilterStandard.simple);
  ...
  return Material(
    child: Container(
      ...
      child: Column(
        ...
        children: [
          const Text('Simple REST API Request:', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
...
Widget apiOutputOData() {
  const requestBuilder = DataGridRequestBuilder(standard: ApiFilterStandard.odata);

  final request = requestBuilder.buildRequest(
    page: 1,
    pageSize: 50,
    filters: {
      'department': const VooDataFilter(operator: VooFilterOperator.equals, value: 'Sales'),
      'salary': const VooDataFilter(operator: VooFilterOperator.between, value: [50000, 100000]),
    },
    sorts: [const VooColumnSort(field: 'salary', direction: VooSortDirection.descending)],
  );
  ...
  return Material(
    child: Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('OData Request:', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
          const SizedBox(height: 8),
          Container(
            padding: const EdgeInsets.all(12),
            decoration: BoxDecoration(
...
Widget apiOutputMongoDB() {
  const requestBuilder = DataGridRequestBuilder(standard: ApiFilterStandard.mongodb);
  ...
  final request = requestBuilder.buildRequest(
    ...
    filters: {
      ...
      'created': const VooDataFilter(operator: VooFilterOperator.greaterThanOrEqual, value: '2024-01-01'),
    },
    sorts: [const VooColumnSort(field: 'created', direction: VooSortDirection.descending)],
  );
  ...
  return Material(
    child: Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('MongoDB Query:', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
          const SizedBox(height: 8),
          Container(
            padding: const EdgeInsets.all(12),
            decoration: BoxDecoration(
...
Widget apiOutputGraphQL() {
  const requestBuilder = DataGridRequestBuilder(standard: ApiFilterStandard.graphql);

  final request = requestBuilder.buildRequest(
    page: 3,
    pageSize: 10,
    filters: {
      'type': const VooDataFilter(operator: VooFilterOperator.equals, value: 'premium'),
      'score': const VooDataFilter(operator: VooFilterOperator.greaterThanOrEqual, value: 4.5),
    },
    sorts: [const VooColumnSort(field: 'score', direction: VooSortDirection.descending)],
  );
  ...
  return Material(
    child: Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('GraphQL Variables:', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
          const SizedBox(height: 8),
          Container(
            padding: const EdgeInsets.all(12),
            decoration: BoxDecoration(
...
class _PrimaryFiltersDataGridPreviewState extends State<PrimaryFiltersDataGridPreview> {
  ...
  void initState() {
    ...
    // Sample order data
    orders = List.generate(100, (index) {
    ...
    controller = VooDataGridController(
      dataSource: dataSource,
      columns: [
        const VooDataColumn<dynamic>(field: 'id', label: 'Order ID', width: 100),
        const VooDataColumn<dynamic>(field: 'customer', label: 'Customer', flex: 2),
        const VooDataColumn<dynamic>(field: 'product', label: 'Product', flex: 2),
        VooDataColumn<dynamic>(
          field: 'amount',
          label: 'Amount',
          width: 100,
        ...
        VooDataColumn<dynamic>(
          ...
          cellBuilder: (context, value, row) {
            ...
            final colors = {'pending': Colors.orange, 'processing': Colors.blue, 'shipped': Colors.purple, 'delivered': Colors.green, 'cancelled': Colors.red};
            final icons = {
            ...
            return Container(
              ...
              decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(12),
                border: Border.all(color: colors[status]?.withValues(alpha: 0.3) ?? Colors.grey),
              ),
              child: Row(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Icon(icons[status], size: 14, color: colors[status]),
                  const SizedBox(width: 4),
                  Text(
                    status.toUpperCase(),
                    style: TextStyle(color: colors[status], fontSize: 11, fontWeight: FontWeight.bold),
                  ),
                ],
              ),
            );
      ...
      child: VooDataGrid(
        ...
        primaryFilters: [
          ...
          PrimaryFilter(
            ...
            filter: const VooDataFilter(operator: VooFilterOperator.equals, value: 'cancelled'),
          ),
        ],
...
class _StatelessDataGridPreviewState extends State<StatelessDataGridPreview> {
  ...
  void initState() {
    ...
    // Define columns
    columns = [
      VooDataColumn<Map<String, dynamic>>(field: 'id', label: 'ID', width: 80, valueGetter: (row) => row['id']),
      VooDataColumn<Map<String, dynamic>>(field: 'name', label: 'Product Name', valueGetter: (row) => row['name']),
      ...
      VooDataColumn<Map<String, dynamic>>(field: 'category', label: 'Category', valueGetter: (row) => row['category']),
    ];
  }
  ...
  Widget build(BuildContext context) => Material(
    child: SizedBox(
      ...
      child: VooDataGridStateless<Map<String, dynamic>>(
        ...
        onToggleFilters: () {
          setState(() {
            currentState = currentState.copyWith(filtersVisible: !currentState.filtersVisible);
          });
        },
        onPageChanged: (page) {
          setState(() {
            ...
            currentState = currentState.copyWith(currentPage: page, rows: products.sublist(start, end));
          });
        },
        onPageSizeChanged: (size) {
          setState(() {
            currentState = currentState.copyWith(pageSize: size, currentPage: 0, rows: products.take(size).toList());
          });
        },
        onSortChanged: (field, direction) {
          setState(() {
            ...
            if (direction != null) {
              newSorts = [VooColumnSort(field: field, direction: direction)];
            }
            ...
            currentState = currentState.copyWith(sorts: newSorts, rows: sortedProducts.sublist(start, end));
          });
        },
        onFilterChanged: (field, filter) {
          setState(() {
        ...
        onRowTap: (row) {
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Tapped: ${row['name']}'), duration: const Duration(seconds: 1)));
        },
      ),
    ),
  );
