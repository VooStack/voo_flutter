...
class DateRangeFilter<T> extends StatelessWidget {
  ...
  const DateRangeFilter({super.key, required this.column, this.currentFilter, required this.onFilterChanged, required this.onFilterCleared});
  ...
  Widget build(BuildContext context) {
    ...
    return TextField(
      ...
      decoration: FilterInputDecoration.standard(
        ...
        suffixIcon: InkWell(
          onTap: () async {
            final range = await showDateRangePicker(context: context, firstDate: DateTime(2000), lastDate: DateTime(2100));
            if (range != null) {
              onFilterChanged({'operator': VooFilterOperator.between, 'value': range.start, 'valueTo': range.end});
            }
          },
