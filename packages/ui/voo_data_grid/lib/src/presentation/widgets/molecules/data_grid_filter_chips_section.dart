...
class DataGridFilterChipsSection<T> extends StatelessWidget {
  ...
  const DataGridFilterChipsSection({super.key, required this.controller, required this.theme});
  ...
  Widget build(BuildContext context) {
    ...
    for (final entry in filters.entries) {
      ...
      try {
        column = controller.columns.firstWhere((col) => col.field == entry.key);
      } catch (_) {
      ...
      filterData[entry.key] = FilterChipData(label: label, value: filter.value, displayValue: displayValue);
    }
