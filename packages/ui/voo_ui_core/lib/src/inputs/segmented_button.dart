...
class VooButtonSegment<T> {
  ...
  const VooButtonSegment({required this.value, required this.label, this.icon, this.tooltip, this.enabled = true});
}
  ...
  Widget build(BuildContext context) {
    ...
    return SegmentedButton<T>(
      segments: segments
          .map(
            (segment) =>
                ButtonSegment<T>(value: segment.value, label: segment.label, icon: segment.icon, tooltip: segment.tooltip, enabled: segment.enabled && enabled),
          )
          .toList(),
      ...
      style: style ?? SegmentedButton.styleFrom(shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(design.radiusMd))),
    );
  }
}
  ...
  Widget build(BuildContext context) {
    ...
    return SegmentedButton<T>(
      segments: segments
          ...
          .toList(),
      selected: selected,
      onSelectionChanged: enabled && onSelectionChanged != null ? onSelectionChanged : null,
    ...
    return Column(
      ...
      children: [
        ...
        VooSegmentedButton<T>(
          ...
          enabled: enabled,
          showSelectedIcon: showSelectedIcon,
          style: hasError ? SegmentedButton.styleFrom(side: BorderSide(color: colorScheme.error)) : null,
        ),
  ...
  Widget build(BuildContext context) {
    ...
    return SegmentedButton<T>(
      segments: segments
          .map(
            (segment) => ButtonSegment<T>(
              value: segment.value,
              icon: Icon(segment.icon, size: iconSize ?? 20),
      ...
      style: SegmentedButton.styleFrom(
        ...
        selectedForegroundColor: selectedColor != null ? colorScheme.onPrimaryContainer : null,
...
class VooIconSegment<T> {
  ...
  const VooIconSegment({required this.value, required this.icon, required this.label, this.tooltip, this.enabled = true, this.showLabel = false});
}
...
class VooCustomSegmentedButton<T> extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: decoration ?? BoxDecoration(color: colorScheme.surfaceContainerHighest, borderRadius: BorderRadius.circular(design.radiusMd)),
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          for (int i = 0; i < segments.length; i++) ...[if (i > 0 && spacing != null) SizedBox(width: spacing), _buildSegment(segments[i], context)],
        ],
      ),
    );
  }
  ...
  Widget _buildSegment(VooCustomSegment<T> segment, BuildContext context) {
    ...
    return Flexible(
      ...
      flex: segment.flex ?? 1,
      child: Material(
        color: isSelected ? (segment.selectedColor ?? colorScheme.primaryContainer) : (segment.unselectedColor ?? Colors.transparent),
        borderRadius: BorderRadius.circular(design.radiusSm),
        child: InkWell(
          onTap: enabled && segment.enabled && onSelectionChanged != null ? () => onSelectionChanged!(segment.value) : null,
          borderRadius: BorderRadius.circular(design.radiusSm),
          child: Padding(
            padding: segment.padding ?? EdgeInsets.symmetric(horizontal: design.spacingMd, vertical: design.spacingSm),
            child: Row(
              mainAxisAlignment: MainAxisAlignment.center,
              mainAxisSize: MainAxisSize.min,
              children: [
                if (segment.leading != null) ...[segment.leading!, SizedBox(width: design.spacingSm)],
                segment.child,
                if (segment.trailing != null) ...[SizedBox(width: design.spacingSm), segment.trailing!],
              ],
            ),
          ),
        ),
...
class VooViewSegment<T> {
  ...
  const VooViewSegment({required this.value, required this.label, this.icon, this.tooltip, this.enabled = true});
}
