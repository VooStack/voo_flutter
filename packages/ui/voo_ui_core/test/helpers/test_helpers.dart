...
Widget createTestApp({required Widget child, ThemeData? theme, Size? screenSize}) => MaterialApp(
...
Widget createResponsiveTestApp({required Widget child, required Size screenSize, ThemeData? theme}) => MaterialApp(
  theme: theme ?? ThemeData(useMaterial3: true),
  home: MediaQuery(
    data: MediaQueryData(size: screenSize),
    child: VooResponsiveBuilder(
...
Future<void> pumpWidgetAndSettle(WidgetTester tester, Widget widget, {Duration? duration}) async {
...
extension WidgetTesterExtensions on WidgetTester {
  ...
  Material getMaterial(Finder finder) => widget<Material>(find.ancestor(of: finder, matching: find.byType(Material)).first);
  ...
  /// Simulates drag
  Future<void> dragAndSettle(Finder finder, Offset offset, {Duration? duration}) async {
...
class GoldenTestHelpers {
  ...
  static Future<void> expectGolden(WidgetTester tester, String name, {Size? size}) async {
    if (size != null) {
      await tester.binding.setSurfaceSize(size);
    }
    await expectLater(find.byType(MaterialApp), matchesGoldenFile(goldenPath(name)));
  }
}
...
class TestData {
  ...
  static Map<String, bool> generateBoolMap(List<String> keys, {bool defaultValue = false}) => Map.fromEntries(keys.map((key) => MapEntry(key, defaultValue)));
