...
class VooValidator {
  ...
  static VooValidationRule<T> required<T>([String? message]) =>
      RequiredValidation<T>(
        errorMessage: message ?? 'This field is required',
      );

  /// Email validator
  static VooValidationRule<String> email([String? message]) =>
      EmailValidation(
        errorMessage: message ?? 'Please enter a valid email address',
      );

  /// Phone number validator
  static VooValidationRule<String> phone([String? message]) =>
      PhoneValidation(
        errorMessage: message ?? 'Please enter a valid phone number',
      );

  /// URL validator
  static VooValidationRule<String> url([String? message]) =>
      UrlValidation(
  ...
  static VooValidationRule<String> minLength(int length, [String? message]) =>
      MinLengthValidation(
        minLength: length,
        errorMessage: message ?? 'Minimum $length characters required',
      );
  ...
  static VooValidationRule<String> maxLength(int length, [String? message]) =>
      MaxLengthValidation(
        maxLength: length,
        errorMessage: message ?? 'Maximum $length characters allowed',
      );

  /// Length range validator
  static VooValidationRule<String> lengthRange(int min, int max, [String? message]) =>
      CompoundValidation<String>(
        rules: [
          minLength(min),
          maxLength(max),
        ],
        errorMessage: message ?? 'Must be between $min and $max characters',
      );

  /// Minimum value validator
  static VooValidationRule<T> min<T extends num>(T value, [String? message]) =>
      MinValueValidation<T>(
        minValue: value,
        errorMessage: message ?? 'Must be at least $value',
      );

  /// Maximum value validator
  static VooValidationRule<T> max<T extends num>(T value, [String? message]) =>
      MaxValueValidation<T>(
        maxValue: value,
        errorMessage: message ?? 'Must be at most $value',
      );
  ...
  /// Pattern validator
  static VooValidationRule<String> pattern(String pattern, String message) =>
      PatternValidation(
        pattern: pattern,
  ...
  static VooValidationRule<T> matches<T>(T value, String message) =>
      CustomValidation<T>(
        validator: (v) => v == value ? null : message,
        errorMessage: message,
      );
  ...
  static VooValidationRule<DateTime> dateRange({
    ...
    String? message,
      ...
      DateRangeValidation(
        minDate: after,
        maxDate: before,
        errorMessage: message ?? 'Date is out of range',
      );

  /// Age validator
  static VooValidationRule<DateTime> age({
    int minAge = 0,
    int maxAge = 150,
    String? message,
  }) {
  ...
  static VooValidationRule<String> alpha({
    ...
    return PatternValidation(
      pattern: pattern,
      errorMessage: message ?? 'Only letters are allowed',
    );
  }

  /// Alphanumeric validator
  static VooValidationRule<String> alphanumeric({
    bool allowSpaces = false,
    String? message,
  }) {
    ...
    return PatternValidation(
      pattern: pattern,
      errorMessage: message ?? 'Only letters and numbers are allowed',
    );
  }
  ...
  static VooValidationRule<String> decimal({
    ...
    return PatternValidation(
      pattern: pattern,
      errorMessage: message ?? 'Invalid decimal number',
    );
  }

  /// Custom validator
  static VooValidationRule<T> custom<T>({
