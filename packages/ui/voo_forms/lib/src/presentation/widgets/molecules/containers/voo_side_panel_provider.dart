...
class VooSidePanelProvider extends StatefulWidget {
  ...
  const VooSidePanelProvider({super.key, required this.child});
...
class VooSidePanelProviderState extends State<VooSidePanelProvider> {
  ...
  Future<dynamic> openSidePanel({required Widget form, String? title, double width = 400, void Function(BuildContext)? onExit}) async {
      ...
      children: [
        ...
        Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border(left: BorderSide(color: Theme.of(context).colorScheme.outline.withValues(alpha: 0.2))),
            boxShadow: [BoxShadow(color: Colors.black.withValues(alpha: 0.1), blurRadius: 8, offset: const Offset(-2, 0))],
          ),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.stretch,
            children: [
              if (_sidePanelTitle != null) _buildHeader(context),
              Expanded(child: _sidePanel ?? const SizedBox.shrink()),
            ],
          ),
        ),
      ],
  ...
  Widget _buildHeader(BuildContext context) {
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          Text(_sidePanelTitle!, style: theme.textTheme.titleLarge),
          IconButton(
            onPressed: () {
              if (_onExit != null) {
                _onExit!(context);
