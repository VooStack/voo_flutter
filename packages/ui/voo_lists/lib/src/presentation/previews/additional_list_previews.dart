...
class _NestedListPreviewState extends State<NestedListPreview> {
  ...
  void _initializeNestedData() {
    items = [
      const ListItem<String>(
        ...
        children: [
          ListItem<String>(
            ...
            isExpandable: true,
            children: [
              ListItem<String>(id: 'child_1_1_1', data: 'main.dart', title: 'main.dart', subtitle: '2.5 KB', leading: Icon(Icons.code)),
              ListItem<String>(id: 'child_1_1_2', data: 'app.dart', title: 'app.dart', subtitle: '1.2 KB', leading: Icon(Icons.code)),
            ],
          ),
          ListItem<String>(
            id: 'child_1_2',
            ...
            isExpandable: true,
            children: [
              ListItem<String>(id: 'child_1_2_1', data: 'widget_test.dart', title: 'widget_test.dart', subtitle: '3.1 KB', leading: Icon(Icons.bug_report)),
            ],
          ),
        ],
      ),
      const ListItem<String>(
        ...
        isExpandable: true,
        children: [ListItem<String>(id: 'child_2_1', data: 'lib', title: 'lib', subtitle: 'Library files', leading: Icon(Icons.folder_open))],
      ),
    ];
  }
  ...
  Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(title: const Text('Nested List Preview')),
    ...
    itemBuilder: (context, index) {
      ...
      if (item.isExpandable && item.children != null) {
        return ExpansionTile(
          ...
          children: [_buildNestedListView(item.children!, depth + 1)],
        );
      } else {
        return ListTile(
          leading: item.leading,
          ...
          child: Column(
            children: [
              ListTile(
                ...
                trailing: Chip(label: Text('${index * 3 + 5} items')),
                onTap: () {
                  setState(() {
                    if (isExpanded) {
                    ...
                    ? Column(
                        children: List.generate(
                          ...
                          (subIndex) => ListTile(
                            ...
                            onTap: () {
                              ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Selected sub-item ${subIndex + 1}')));
                            },
                          ),
                        ),
                      )
...
class _SearchableListPreviewState extends State<SearchableListPreview> {
  ...
  void _initializeData() {
    allItems = List.generate(
      ...
      (index) => ListItem<String>(
        ...
        leading: Icon([Icons.computer, Icons.book, Icons.checkroom, Icons.restaurant, Icons.sports_basketball][index % 5]),
      ),
    );
    ...
    setState(() {
      ...
      if (searchQuery.isEmpty) {
        ...
        filteredItems = allItems
            .where((item) => item.title.toLowerCase().contains(searchQuery) || (item.subtitle?.toLowerCase().contains(searchQuery) ?? false))
            .toList();
      }
    });
  }
  ...
  @override
  Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(title: const Text('Searchable List Preview')),
    body: Column(
      children: [
        Container(
          ...
          child: TextField(
            ...
            decoration: InputDecoration(
              ...
              suffixIcon: searchQuery.isNotEmpty ? IconButton(icon: const Icon(Icons.clear), onPressed: searchController.clear) : null,
        ...
        Padding(
          ...
          child: Row(
            ...
            children: [
              Text('${filteredItems.length} items found', style: Theme.of(context).textTheme.bodySmall),
              if (searchQuery.isNotEmpty) TextButton(onPressed: searchController.clear, child: const Text('Clear filter')),
            ],
          ),
        ),
        Expanded(
              ...
              ? Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text('No items match "$searchQuery"', style: Theme.of(context).textTheme.titleMedium),
...
class _InfiniteListPreviewState extends State<InfiniteListPreview> {
  ...
  Future<void> _loadMoreItems() async {
    ...
    setState(() {
      ...
      final newItems = List.generate(itemsPerPage, (index) {
        ...
        return ListItem<String>(
          ...
          leading: CircleAvatar(child: Text('${itemIndex + 1}')),
        );
      });
    ...
    body: ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return ListTile(
          ...
          title: Text(item.title),
          subtitle: Text(item.subtitle ?? ''),
          trailing: Text('#${index + 1}', style: Theme.of(context).textTheme.bodySmall),
        );
      },
    ),
  );
...
class _PaginatedListPreviewState extends State<PaginatedListPreview> {
  ...
  Future<void> _loadPage(int page) async {
    ...
    setState(() {
      ...
      currentPageItems = List.generate(itemsPerPage, (index) {
        final itemIndex = startIndex + index;
        return ListItem<String>(
  ...
  @override
  Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(title: const Text('Paginated List Preview')),
    body: Column(
      children: [
        Container(
          padding: const EdgeInsets.all(8),
          ...
          child: Row(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              IconButton(icon: const Icon(Icons.first_page), onPressed: currentPage > 1 && !isLoading ? () => _loadPage(1) : null),
              IconButton(icon: const Icon(Icons.chevron_left), onPressed: currentPage > 1 && !isLoading ? () => _loadPage(currentPage - 1) : null),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
                ...
                child: Text('Page $currentPage of $totalPages', style: Theme.of(context).textTheme.bodyMedium),
              ),
              IconButton(icon: const Icon(Icons.chevron_right), onPressed: currentPage < totalPages && !isLoading ? () => _loadPage(currentPage + 1) : null),
              IconButton(icon: const Icon(Icons.last_page), onPressed: currentPage < totalPages && !isLoading ? () => _loadPage(totalPages) : null),
            ],
          ),
        ),
        Expanded(
              ...
              : ListView.separated(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return ListTile(
                      ...
                      title: Text(item.title),
                      subtitle: Text(item.subtitle ?? ''),
                      trailing: Text('${(currentPage - 1) * itemsPerPage + index + 1}', style: Theme.of(context).textTheme.bodySmall),
                      onTap: () {
                        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Selected: ${item.title}')));
                      },
                    );
          ...
          child: Wrap(
            ...
            children: List.generate(totalPages, (index) {
              ...
              return ActionChip(
                ...
                labelStyle: TextStyle(color: page == currentPage ? Theme.of(context).colorScheme.onPrimary : null),
              );
            }),
          ),
        ),
...
class _NestedReorderableListPreviewState extends State<NestedReorderableListPreview> {
  ...
  void _handleReorder(List<ListItem<String>> reorderedItems) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('List reordered'), duration: Duration(seconds: 1)));
  }

  void _handleItemMoved(ListItem<String> item, ListItem<String>? newParent) {
    final parentTitle = newParent?.title ?? 'root level';
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Moved "${item.title}" to $parentTitle'), duration: const Duration(seconds: 2)));
  }
  ...
  Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(
      ...
      actions: [
        ...
        IconButton(icon: const Icon(Icons.refresh), onPressed: _initializeData, tooltip: 'Reset list'),
      ],
    ),
    body: Column(
      children: [
          ...
          child: VooNestedReorderableList<String>(
            ...
            onItemTap: (item) {
              if (item.children == null || item.children!.isEmpty) {
                ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Selected: ${item.title}'), duration: const Duration(seconds: 1)));
              }
            },
          ),
        ),
...
class AllListsShowcase extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('VooLists Showcase')),
      body: ListView.builder(
        padding: const EdgeInsets.all(8),
        ...
        itemBuilder: (context, index) {
          ...
          return Card(
            ...
            child: ListTile(
              ...
              onTap: () {
                ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Navigate to $title preview')));
              },
            ),
          );
        },
